...
userRouter.post(
  ...
  body("password2").custom((password2, { req }) => {
    return validateSignUpPasswords(req.body.password1, password2);
  }),
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ serverError: "Could not create user due to server error." });
    }
...
userRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!userId)
      return res.status(401).json({ message: "User e-mail does not exists!" });
    const isCorrect: any = await isUserPasswordCorrect(
      req.body.password,
      req.body.email
    );
    if (!isCorrect)
...
userRouter.get(
  ...
  async (req: Request, res: Response) => {
    return res
      .clearCookie("jwt")
      .status(200)
      .json({ success: "The user has successfully logged out!" });
  }
);
...
userRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    return res.status(200).json({
      isOtpValid: verifyTOTP(
        req.body.token,
        req.body.authenticatedId,
        "pwdreset"
      ),
    });
  }
...
userRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const isOTPValid: boolean = verifyTOTP(
      req.body.token,
      req.body.authenticatedId,
      "pwdreset"
    );
    ...
    return res
      .clearCookie("jwt")
      .status(200)
      .json({ msg: "Password was succesfully reseted! Please, login again." });
  }
);
...
userRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    return res
      .status(200)
      .json({ isOtpValid: verifyTOTP(req.body.token, userId, "pwdforgot") });
  }
);
